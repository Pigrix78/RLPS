...
def train():
    ...
    for epoch in tqdm(range(opt.max_epoch)):
        ...
        for i, data in enumerate(train_loader):
            ...
            classifier.train()
            pred, gcn = classifier(points, adj)  

            tensor_empty = torch.Tensor([]).cuda()
            for target_index in target:
                tensor_empty = torch.cat((tensor_empty, gcn[target_index.item()].float().cuda()), 0)
            label_mse_tensor = tensor_empty.view(-1, classes).cuda()

            one_hot_target = torch.nn.functional.one_hot(target, int(classes)).float()
            loss = F.mse_loss(pred, one_hot_target) + F.mse_loss(pred, label_mse_tensor)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def test(num):
    ...
    for i, data in enumerate(test_loader):
        ...
        classifier.eval()
        pred_choice, _ = classifier(points, adj)
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        train_loader, test_loader, train_idx, test_idx = load_hyperdata(classes, feature, label, process_path, opt.w, opt.train_rate, opt.tr_batch_size, opt.te_batch_size)

        adj = getAdj(gcn_data[train_idx], label[train_idx], classes)
        ...
        classifier = PSGCN(feature.shape[-1], classes).cuda()
