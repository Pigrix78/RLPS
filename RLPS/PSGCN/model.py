...
class PSGCN(nn.Module):
    def __init__(self, input_channel, k=2):
        ...
        self.AttenMLP = nn.ModuleList([
            Block(256, num_heads=4),  
            Block(256, num_heads=8)  
        ])
        ...
        self.gcn1 = GraphConvolution(256, 64)
        self.gcn2 = GraphConvolution(64, k)
    ...
    def forward(self, x, adj):
        ...
        for encoder in self.AttenMLP:
            x = encoder(x) 
        x = x.permute(0, 2, 1).view(-1, 256, 7, 7) 
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        ...
        gcn_data = self.fc3.weight 
        y = F.relu(self.gcn1(gcn_data, adj))
        y = self.gcn2(y, adj) 
